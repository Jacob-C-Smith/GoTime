...
case $# in 
1) ;;
*) echo "Usage: " `basename $0` " <server-quantity>"; exit 1;;
esac
...
do
    ...
    echo "${SERVER_ADDRESSES[i]}" >> src/server/servers.txt
    echo "" > "logs/time_server_${i}.txt" 
done

# Build the docker image
sudo docker build -t time_server . > logs/docker_build_output.txt 2> logs/docker_build_output.txt

# Start identity servers
for (( i=0; i < $SERVER_QUANTITY; i++ ))
do
    printf "${GREEN}Starting time server $(($i + 1))${RESET}\r"
    sudo docker logs --details -f $(sudo docker run -it -d time_server) >& "logs/time_server_${i}.txt" &
done
